...
- [ ] 2.2 Скрипт автоматически проверит и предложит установить:
  ...
  - protoc
  - protoc-gen-go, protoc-gen-go-grpc (Go плагины для protoc)
  - protoc-gen-grpc-web (gRPC-Web плагин для frontend)
  - buf
  - jq (опционально)
...
- [ ] 6.2 Проверить минимальные версии: Node.js >= 20, Go = 1.22.x
- [ ] 6.2b Если Go > 1.22.x — предупредить что YC поддерживает только 1.22 и `go mod tidy` будет перезаписывать go.mod
