...
- **Go 1.22 строго** — YC Serverless Containers не поддерживает выше. `go mod tidy` может перезаписать версию в `go.mod` — проверяйте после каждого запуска.
- **ENCRYPTION_KEY** — хранится в Lockbox как заготовка, но пока не используется в коде. Подключите при необходимости шифрования PII.
- **gRPC-Web** — токены хранятся в cookies (не httpOnly), это ограничение протокола gRPC-Web.
- **Спеки 3.1 и 3.2 взаимоисключающие** — выберите одну. Команды из `tech.md` для невыбранного режима не будут работать.
- **Proto wrapper** — при добавлении новых gRPC методов нужно вручную обновлять `*_pb_wrapper.ts`. Без этого метод молча не работает.
